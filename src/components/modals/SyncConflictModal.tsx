...
function SyncConflictModal() {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-(--color-bg-shadow)/40 backdrop-blur-[2px] p-4">
            <div
                className="bg-(--color-bg-main) rounded-2xl w-full max-w-md mx-4 shadow-xl border border-(--color-border)"
                role="dialog"
                aria-modal="true"
            ...
            >
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="p-4 bg-(--color-bg-input-secondary) rounded-xl border border-(--color-border)">
                        <div className="text-sm space-y-3">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="text-center">
                                    <div className="text-(--color-text-interface) font-medium mb-1">
                                        {t('settings.others.sync.conflict.local')}
                                    </div>
                                    <div className="font-mono text-(--color-text-primary) bg-(--color-bg-hover) px-3 py-2 rounded-md">
                                        {localPatchSeq}
                                    </div>
                                </div>
                                <div className="text-center">
                            ...
                            <div className="text-center text-(--color-text-secondary) font-medium">
                                {comparisonText}
                            </div>
                            <div className="text-center">
                                <div className="text-(--color-text-interface) font-medium mb-1">
                                ...
                                <div className="font-mono text-xs text-(--color-text-primary) bg-(--color-bg-hover) px-2.5 py-1 rounded-md">
                    ...
                    <div className="flex flex-col gap-2.5 pt-2">
                        <button
                            ...
                            className="w-full px-4 py-3 bg-(--color-button-neutral) text-(--color-text-accent) rounded-xl hover:bg-(--color-button-neutral-hover) transition-colors font-medium"
                        >
                            {t('settings.others.sync.conflict.keepLocal')}
                        </button>
                        <button
                            onClick={handleApplyServer}
                            className="w-full px-4 py-3 bg-(--color-bg-input-secondary) text-(--color-text-primary) rounded-xl hover:bg-(--color-bg-hover) transition-colors font-medium border border-(--color-border)"
                        >
