...
function EditGroupChatModal({ isOpen, onClose }: EditGroupChatModalProps) {
    ...
    return (
        <>
            <div className="fixed inset-0 bg-(--color-bg-shadow)/50 flex items-center justify-center z-50 p-4">
                <div className="bg-(--color-bg-main) rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl">
                    <div className="sticky top-0 bg-(--color-bg-main) px-6 py-4 border-b border-(--color-border) rounded-t-2xl">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xl font-semibold text-(--color-text-primary)">{t('main.group.editModal.title')}</h2>
                            <button onClick={handleClose} className="p-2 text-(--color-icon-tertiary) hover:text-(--color-icon-primary) hover:bg-(--color-bg-hover) rounded-full transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="p-6 space-y-6">
                        <div>
                            <label className="block text-sm font-medium text-(--color-text-interface) mb-2">{t('main.group.editModal.nameLabel')}</label>
                            <input type="text" value={name} onChange={e => setName(e.target.value)}
                                className="w-full p-3 bg-(--color-bg-input-secondary) text-(--color-text-primary) rounded-xl border border-(--color-border) focus:border-(--color-focus-border) focus:ring-2 focus:ring-(--color-focus-border)/50 focus:outline-none" />
                        </div>

                        <div className="space-y-4">
                            <h3 className="text-lg font-semibold text-(--color-text-primary)">{t('main.group.editModal.response.title')}</h3>
                            <div>
                                <label className="block text-sm font-medium text-(--color-text-interface) mb-2">
                                    <span>{t('main.group.editModal.response.roomFreq', { value: Math.round(settings.responseFrequency * 100) })}</span>
                                    <span className="text-xs text-(--color-text-secondary) ml-2">{t('main.group.editModal.response.freqHelp')}</span>
                                </label>
                                <input type="range" min="0" max="100" value={Math.round(settings.responseFrequency * 100)}
                                    ...
                                    className="w-full h-2 bg-(--color-bg-secondary-accent) rounded-lg appearance-none cursor-pointer slider-thumb" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-(--color-text-interface) mb-2">{t('main.group.editModal.response.maxLabel')}</label>
                                <div className="relative">
                                    <input type="number" min="1" max={participants.length} value={settings.maxRespondingCharacters}
                                        ...
                                        className="w-full p-3 pr-10 bg-(--color-bg-input-secondary) text-(--color-text-primary) rounded-xl border border-(--color-border) focus:border-(--color-focus-border) focus:ring-2 focus:ring-(--color-focus-border)/50 focus:outline-none" />
                                    <span className="absolute inset-y-0 right-3 flex items-center text-(--color-text-secondary)">{t('units.peopleSuffix')}</span>
                                </div>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <h3 className="text-lg font-semibold text-(--color-text-primary)">{t('main.group.editModal.participants.title')}</h3>
                            <div className="space-y-4">
                                {participants.map(participant => (
                                    <div key={participant.id} className="space-y-2 max-h-60 overflow-y-auto">
                                        <label className="flex flex-col p-3 bg-(--color-bg-input-secondary) rounded-xl hover:bg-(--color-bg-hover) cursor-pointer transition-colors border border-(--color-border-secondary)">
                                            <div className="flex items-center flex-1 mb-2">
                                                <div className="flex items-center gap-3 flex-1">
                                                    <Avatar char={participant} size="md" />
                                                    <div>
                                                        <div className="font-medium text-(--color-text-primary)">{participant.name}</div>
                                                    </div>
                                                </div>
                                                <input
                                                    ...
                                                    className="group-chat-participant mr-3 text-(--color-button-primary-accent) bg-(--color-bg-main) border-(--color-border-strong) rounded focus:ring-(--color-focus-border)"
                                                />
                                                {t('main.group.editModal.participants.activate')}
                                                <button
                                                    type="button"
                                                    className="ml-2 opacity-100 transition-opacity duration-200 p-1 bg-(--color-button-negative) hover:bg-(--color-button-negative-accent) rounded-full text-(--color-text-accent)"
                                            ...
                                            <div className="flex-1">
                                                <label className="block text-sm font-medium text-(--color-text-interface)">
                                                    ...
                                                    <span className="text-xs text-(--color-text-secondary) ml-2">{t('main.group.editModal.participants.probHelp')}</span>
                                                </label>
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-(--color-border)">
                            <button onClick={handleInvite} className="px-6 py-2 bg-(--color-button-positive) hover:bg-(--color-button-positive-accent) text-(--color-text-accent) rounded-xl transition-colors font-medium mr-auto">{t('main.group.editModal.actions.invite')}</button>
                            <button onClick={handleClose} className="px-6 py-2 bg-(--color-button-secondary) hover:bg-(--color-button-secondary-accent) text-(--color-text-interface) rounded-xl transition-colors font-medium">{t('common.cancel')}</button>
                            <button onClick={handleSave} className="px-6 py-2 bg-(--color-button-primary) hover:bg-(--color-button-primary-accent) text-(--color-text-accent) rounded-xl transition-colors font-medium">{t('common.save')}</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="fixed inset-0 bg-(--color-bg-shadow)/40 flex items-center justify-center z-50 p-4">
                    <div className="bg-(--color-bg-main) rounded-2xl w-full max-w-md mx-4 shadow-xl max-h-[80vh] flex flex-col">
                        <div className="p-6 border-b border-(--color-border) flex items-center justify-between">
                            <h3 className="text-lg font-semibold text-(--color-text-primary)">{t('main.group.editModal.invite.title')}</h3>
                            <button onClick={handleInviteCancel} className="p-2 hover:bg-(--color-bg-hover) rounded-full transition-colors">
                        ...
                        <div className="p-6 space-y-2 overflow-y-auto flex-1">
                            {uninvitedCharacters.length === 0 ? (
                                <div className="text-(--color-text-secondary) text-center">{t('main.group.editModal.invite.empty')}</div>
                            ) : (
                                uninvitedCharacters.map(char => (
                                    <label key={char.id} className={`flex items-center p-3 bg-(--color-bg-input-secondary) rounded-xl hover:bg-(--color-bg-hover) cursor-pointer transition-colors border border-(--color-border-secondary) ${selectedInviteId === char.id ? 'ring-2 ring-(--color-focus-border)' : ''}`}>
                                        <input
                                            type="radio"
                                            ...
                                            className="mr-3 text-(--color-button-primary-accent) bg-(--color-bg-main) border-(--color-border-strong) rounded focus:ring-(--color-focus-border)"
                                        />
                                        <div className="flex items-center gap-3 flex-1">
                                            <Avatar char={char} size="md" />
                                            <div>
                                                <div className="font-medium text-(--color-text-primary)">{char.name}</div>
                                            </div>
                                        </div>
                                    </label>
                                ))
                        ...
                        <div className="p-6 border-t border-(--color-border) flex gap-3">
                            <button onClick={handleInviteCancel} className="flex-1 py-2 px-4 bg-(--color-button-secondary) hover:bg-(--color-button-secondary-accent) text-(--color-text-interface) rounded-xl transition-colors font-medium">{t('common.cancel')}</button>
                            <button
                                onClick={handleInviteConfirm}
                                className="flex-1 py-2 px-4 bg-(--color-button-primary) hover:bg-(--color-button-primary-accent) text-(--color-text-accent) rounded-xl transition-colors disabled:bg-(--color-button-disabled) disabled:text-(--color-icon-tertiary) font-medium"
            ...
            {leaveDialogOpen && (
                <div className="fixed inset-0 bg-(--color-bg-shadow)/40 flex items-center justify-center z-50 p-4">
                    <div className="bg-(--color-bg-main) rounded-2xl w-full max-w-md mx-4 shadow-xl max-h-[80vh] flex flex-col">
                        <div className="p-6 border-b border-(--color-border) flex items-center justify-between">
                            <h3 className="text-lg font-semibold text-(--color-text-primary)">{t('main.group.editModal.leave.title')}</h3>
                            <button onClick={handleLeaveCancel} className="p-2 hover:bg-(--color-bg-hover) rounded-full transition-colors">
                                <X className="w-5 h-5 text-(--color-icon-tertiary)" />
                            </button>
                        </div>
                        <div className="p-6 space-y-4 overflow-y-auto flex-1">
                            <label className="block text-m font-medium text-(--color-text-interface) mb-5">{t('main.group.editModal.leave.whoLabel', { name: allCharacters.find(c => c.id === leavingCharId)?.name || t('main.group.editModal.unknown') })}</label>
                            <select
                                className="w-full p-3 bg-(--color-bg-input-secondary) text-(--color-text-primary) rounded-xl border border-(--color-border) focus:border-(--color-focus-border) focus:ring-2 focus:ring-(--color-focus-border)/50 focus:outline-none"
                                value={leaveReason}
                            ...
                            {leaveReason === t('main.group.editModal.customInput') && (
                                <>
                                    <textarea
                                        className="w-full p-3 bg-(--color-bg-input-secondary) text-(--color-text-primary) rounded-xl border border-(--color-border) focus:border-(--color-focus-border) focus:ring-2 focus:ring-(--color-focus-border)/50 focus:outline-none min-h-[80px]"
                        ...
                        <div className="p-6 border-t border-(--color-border) flex gap-3">
                            <button onClick={handleLeaveCancel} className="flex-1 py-2 px-4 bg-(--color-button-secondary) hover:bg-(--color-button-secondary-accent) text-(--color-text-interface) rounded-xl transition-colors font-medium">{t('common.cancel')}</button>
                            <button
                                onClick={handleLeaveConfirm}
                                className="flex-1 py-2 px-4 bg-(--color-button-negative) hover:bg-(--color-button-negative) text-(--color-text-accent) rounded-xl transition-colors font-medium"
