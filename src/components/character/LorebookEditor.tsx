    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h5 className="text-sm font-medium text-(--color-text-interface)">{t('characterPanel.lorebookEditor.header', { count: sorted.length })}</h5>
                <button onClick={addLore} className="py-2 px-3 bg-(--color-button-primary) hover:bg-(--color-button-primary-accent) text-(--color-text-accent) rounded-lg text-sm inline-flex items-center gap-2 transition-colors">
                    <Plus className="w-4 h-4" /> {t('characterPanel.lorebookEditor.add')}
                </button>
            </div>

            {sorted.map((l, i) => (
                <div key={l.id} className="bg-(--color-bg-main) border border-(--color-border) rounded-lg shadow-sm">
                    <div className="flex flex-wrap items-center gap-2 p-4">
                        <GripVertical className="w-4 h-4 text-(--color-icon-secondary)" />
                        <span className="text-xs text-(--color-text-secondary) w-8 font-medium">#{i + 1}</span>
                        <input
                            value={l.name}
                            onChange={e => updateLore(l.id, { name: e.target.value })}
                            placeholder={t('characterPanel.lorebookEditor.namePlaceholder')}
                            className="min-w-0 flex-1 basis-full sm:basis-auto px-3 py-2 bg-(--color-bg-input-secondary) text-(--color-text-primary) rounded-lg border border-(--color-border) focus:ring-2 focus:ring-(--color-focus-border)/50 focus:border-(--color-focus-border) text-sm"
                        />
                        <div className="flex gap-1 w-full justify-end sm:w-auto">
                            <button onClick={() => moveLore(l.id, -1)} className="p-2 rounded-lg bg-(--color-button-secondary) hover:bg-(--color-button-secondary-accent) text-(--color-icon-primary) disabled:opacity-40 transition-colors" disabled={i === 0} title={t('characterPanel.lorebookEditor.moveUpTitle')}>
                                <ChevronUp className="w-4 h-4" />
                            </button>
                            <button onClick={() => moveLore(l.id, 1)} className="p-2 rounded-lg bg-(--color-button-secondary) hover:bg-(--color-button-secondary-accent) text-(--color-icon-primary) disabled:opacity-40 transition-colors" disabled={i === sorted.length - 1} title={t('characterPanel.lorebookEditor.moveDownTitle')}>
                                <ChevronDown className="w-4 h-4" />
                            </button>
                            <button onClick={() => setExpandedId(expandedId === l.id ? null : l.id)} className="p-2 rounded-lg bg-(--color-button-primary) hover:bg-(--color-button-primary-accent) text-(--color-text-accent) transition-colors" title={expandedId === l.id ? t('characterPanel.lorebookEditor.collapse') : t('characterPanel.lorebookEditor.edit')}>
                                <Pencil className="w-4 h-4" />
                            </button>
                            <button onClick={() => removeLore(l.id)} className="p-2 rounded-lg bg-(--color-button-negative) hover:bg-(--color-button-negative) text-(--color-text-accent) transition-colors" title={t('characterPanel.lorebookEditor.delete')}>
                    ...
                    {expandedId === l.id && (
                        <div className="p-4 pt-0 space-y-4 border-t border-(--color-border)">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="sm:col-span-2">
                                    <label className="text-sm font-medium text-(--color-text-interface) mb-2 block">{t('characterPanel.lorebookEditor.activationKeysLabel')}</label>
                                    <div className="flex flex-col gap-2">
                                        <input
                                            ...
                                            className={`w-full min-w-0 px-3 py-2 bg-(--color-bg-input-secondary) text-(--color-text-primary) rounded-lg border border-(--color-border) focus:ring-2 focus:ring-(--color-focus-border)/50 focus:border-(--color-focus-border) text-sm ${l.alwaysActive ? 'opacity-50 cursor-not-allowed' : ''}`}
                            ...
                            <div>
                                <label className="text-sm font-medium text-(--color-text-interface) mb-2 block">{t('characterPanel.lorebookEditor.promptLabel')}</label>
                                <textarea
                                    value={l.prompt}
                            ...
                            <div className="flex flex-col gap-3 mt-2">
                                <label className="flex items-center gap-3 text-sm text-(--color-text-interface) cursor-pointer">
                                    <div className="relative">
                                        ...
                                        <div className="w-5 h-5 bg-(--color-button-secondary-accent) border border-(--color-border-strong) rounded peer-checked:bg-(--color-focus-border) peer-checked:border-(--color-focus-border) transition-colors"></div>
                                        <svg className="absolute top-0.5 left-0.5 w-3 h-3 text-(--color-text-accent) opacity-0 peer-checked:opacity-100 transition-opacity" fill="currentColor" viewBox="0 0 20 20">
            ...
            {sorted.length === 0 && (
                <div className="text-center text-sm text-(--color-icon-tertiary) border-2 border-dashed border-(--color-border) rounded-lg p-8 bg-(--color-bg-secondary)">
                    <div className="flex flex-col items-center gap-2">
                        <div className="w-12 h-12 bg-(--color-button-secondary-accent) rounded-full flex items-center justify-center">
                            <Plus className="w-6 h-6 text-(--color-icon-secondary)" />
                        </div>
                        <p className="font-medium">{t('characterPanel.lorebookEditor.emptyTitle')}</p>
                        <p className="text-xs text-(--color-text-informative-secondary)">{t('characterPanel.lorebookEditor.emptyDescription')}</p>
                    </div>
                </div>
            )}
        </div>
