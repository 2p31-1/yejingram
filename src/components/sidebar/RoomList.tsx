...
function RoomList({
    ...
    return (
        <div
            className={`chat-room-item group relative cursor-pointer transition-transform duration-200 select-none ${isSelected ? 'bg-(--color-bg-roomlist) border-l-4 border-(--color-focus-border)' : 'hover:bg-(--color-bg-secondary)'
                }`}
        ...
        >
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                            <h4 className={`text-sm font-medium truncate ${isSelected ? 'text-(--color-text-roomlist-title)' : 'text-(--color-text-primary)'}`}>
                                {room.name}
                            </h4>
                            {room.proactiveEnabled && (
                                <div title={t('main.input.proactiveEnabled')}>
                                    <BellRing
                                        className="w-3.5 h-3.5 text-(--color-button-primary) shrink-0"
                                    />
                                </div>
            ...
            <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex space-x-1 z-10">
                <button
                    ...
                    className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-1 bg-(--color-bg-roomactions-positive) hover:bg-(--color-bg-roomactions-positive-hover) rounded-full text-(--color-button-primary-accent)"
                ...
                <button
                    ...
                    className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-1 bg-(--color-bg-roomactions-negative) hover:bg-(--color-bg-roomactions-negative-hover) rounded-full text-(--color-button-negative-accent)"
