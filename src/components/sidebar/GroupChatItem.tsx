...
function GroupChatItem({ room, setRoomId, isSelected, openEditGroupChatModal, useDoubleClick = false }: GroupChatItemProps) {
    ...
    return (
        <div
            className={`relative group cursor-pointer transition-all duration-200 px-4 py-3 select-none ${isSelected ? 'bg-(--color-bg-roomlist) border-l-4 border-(--color-focus-border)' : 'hover:bg-(--color-bg-secondary)'
                }`}
            ...
            <div className="flex items-center space-x-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                        <h4 className={`font-semibold truncate text-sm ${isSelected ? 'text-(--color-text-roomlist-title)' : 'text-(--color-text-primary)'
                            }`}>
                            {room.name}
                        </h4>
