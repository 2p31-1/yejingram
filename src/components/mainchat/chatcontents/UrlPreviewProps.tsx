...
export const UrlPreview: React.FC<UrlPreviewProps> = ({ url }) => {
    ...
    return (
        <div className="mt-2 p-3 border border-(--color-border-strong) rounded-lg bg-(--color-bg-main) hover:bg-(--color-bg-secondary) transition-colors max-w-md cursor-pointer" onClick={() => window.open(siteUrl, '_blank')}>
            {image && !imageError && (
                <img
            ...
            <div className="text-sm">
                {title && <div className="font-semibold text-(--color-text-primary) mb-1">{title}</div>}
                {description && <div className="text-(--color-text-tertiary) line-clamp-2">{description}</div>}
                <div className="text-(--color-text-secondary) text-xs mt-1 truncate">{siteUrl}</div>
            </div>
        </div>
    );
};